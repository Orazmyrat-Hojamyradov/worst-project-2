...
import { Home, Search, Heart, User, GraduationCap, Menu, X, LogIn } from "lucide-react";
  ...
  return (
    <header className={styles.header}>
      ...
      <nav className={`${styles.nav} ${isOpen ? styles.show : ""}`}>
        ...
        <Link href="/search" className={styles.navItem} onClick={() => setIsOpen(false)}>
          <Search size={18} />
          <span>{t('nav.search')}</span>
        </Link>
        <Link href="/favourites" className={styles.navItem} onClick={() => setIsOpen(false)}>
          <Heart size={18} />
